import { HackAction } from 'lib/hacking.js';
import { formatBytes } from 'lib/general.js';
...
export async function runScript(ns, server, script, scriptArgs) {
	...
	scriptArgs = Array.isArray(scriptArgs) ? scriptArgs : [scriptArgs];
	if (threads > 0) {
		if(scriptArgs) {
		...
		const message = `Script ${script} (${formatBytes(ns.getScriptRam(script)*10**9)} RAM) deployed with
...
export async function runScriptMaxThreads(ns, server, script, scriptArgs) {
	...
	if (threads > 0) {
		...
		const message = `Script (${formatBytes(ns.getScriptRam(script)*10**9)} RAM) deployed on 
...
export async function getRatio(ns, server) {

	const actions = [HackAction.Grow, HackAction.Weaken, HackAction.Hack];
	const calculationPromises = actions.map(action => action.calculate(ns, server));
	const calculations = await Promise.all(calculationPromises);

	ns.tprint(`Inspecting ${server}...`)
	calculations.forEach((calc, index) => {
		const action = actions[index];
		ns.tprint(`\t ${action.name}: ${parseInt(calc.threads)} threads | ${(calc.time/1000/60).toFixed(2)} minutes`);
	})
